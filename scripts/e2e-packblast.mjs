...
const base = process.env.E2E_URL || 'http://localhost:5173/packblast/';
mkdirSync('e2e-out/packblast', { recursive: true });
const report = [];
...
async function playBattle(page, name, opts = {}) {
  ...
  if (opts.screenshotAt) { await page.waitForTimeout(opts.screenshotAt); await page.screenshot({ path: `e2e-out/packblast/${name}-${opts.tag || ('battle' + stage)}.png` }); }
  // 충격파: 위험할 때 한 번
  const t0 = Date.now();
...
async function afterBattle(page, name, opts = {}) {
  ...
  if (opts.screenshotSummary) await page.screenshot({ path: `e2e-out/packblast/${name}-summary${opts.stage}.png` });
  ...
  if (s.phase === 'unlock') {
    ...
    if (opts.screenshotUnlock) await page.screenshot({ path: `e2e-out/packblast/${name}-unlock.png` });
    await tapText(page, '해금 확정');
    s = await state(page);
    log(`${name}: unlock → locked=${s.locked} phase=${s.phase}`);
  }
  if (s.phase === 'reward') {
    if (opts.screenshotReward) await page.screenshot({ path: `e2e-out/packblast/${name}-reward${opts.stage}.png` });
...
async function run(name, viewport, mode) {
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-01-title.png` });
  // ---- 튜토리얼: 실제 탭으로 배치 ----
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-02-preview.png` });
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-03-invalid.png` });
  // 회전 후 기관총 옆에 배치 (2,0)
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-04-links.png` });
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-05-prep-ready.png` });
  // 정리 중 새로고침 → 장비·재화 일치
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-06-battle1.png` });
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-07-pause.png` });
  ...
  await page.screenshot({ path: `e2e-out/packblast/${name}-09-prep2.png` });
  ...
  if (mode === 'short') {
    ...
    if (mt.includes('작업대')) { await page.screenshot({ path: `e2e-out/packblast/${name}-10-bench-confirm.png` }); await tapText(page, '돌아가서 정리'); const s2 = await state(page); log(`${name}: cancel keeps items=${s2.items.length} phase=${s2.phase}`); }
    await botPrep(page);
    ...
    await page.screenshot({ path: `e2e-out/packblast/${name}-12-prep3.png` });
  } else {
  ...
  await tapText(page, '도감'); await page.screenshot({ path: `e2e-out/packblast/${name}-30-codex.png` }); await page.tap('#overlay .x');
  await tapText(page, '설정'); await page.screenshot({ path: `e2e-out/packblast/${name}-31-settings.png` }); await page.tap('#overlay .x');
  await ctx.close();
...
await browser.close();
writeFileSync('e2e-out/packblast/report.txt', report.join('\n'));
