...
self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(ks => Promise.all(ks.filter(k => k.startsWith('crimson-exile-') && k !== VERSION).map(k => caches.delete(k)))).then(() => self.clients.claim())); });
