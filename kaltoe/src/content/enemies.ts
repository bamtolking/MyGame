...
// 적 로스터 — 칼퇴 서바이버
//
// 체력 설계 기준 (BRIEF 밸런스 목표):
//  - hp는 '시간 배율 적용 전' 기본값. 실제 체력 = hp × 구간 hpMul(0초 1.0 → 600초 10) × 스테이지 hpMul
//    (office 1 / crunch 1.35 / dinner 1.7 / holiday 2.1) × 저주·야근 강도.
...
//    실제 = hp × 스테이지 hpMul × 저주·야근 강도. 최종 보스는 원래 설계(기본 × 등장 시점 구간 배율 8)를 hp에 미리 곱해 두었다.
//    목표 '그 시점 예상 DPS × 30~40초'. 중간 보스는 봇 측정상 ×1로도 처치 40~55초라 배율 없이 둔다.
//      team_lead 2200  ≈  2.2k  |  bujang      44000 ≈  44k   (office, ×1.0)
//      pm        2400  ≈  3.2k  |  director    48000 ≈  65k   (crunch, ×1.35)
//      toast     2600  ≈  4.4k  |  bomb_bujang 52000 ≈  88k   (dinner, ×1.7)
//      uncle     2800  ≈  5.9k  |  big_aunt    56000 ≈ 118k   (holiday, ×2.1)
...
const OFFICE: EnemyDef[] = [
  ...
  // 엘리트
  {
    ...
    elite: true,
    behavior: 'spawner', params: { spawnCd: 4, spawnId: 'memo', spawnCount: 3 },
    abilities: [
      { kind: 'ring', cooldown: 6, params: { bullets: 8, speed: 130, damage: 7 }, shout: '용지 걸림! 용지 걸림!' },
    ],
    intro: '고장난 프린터가 서류를 토해냅니다. 부수면 상자!',
  },
  // 중간 보스
  {
    id: 'team_lead', name: '팀장님', sprite: '😒', tint: '#2b6cb0',
    hp: 2200, speed: 50, damage: 20, radius: 40, xp: 60, knockbackResist: 1,
    boss: true, body: { suit: '#4a5568', tie: '#3182ce' },
    behavior: 'chase',
    abilities: [
      { kind: 'summon', cooldown: 8, params: { enemy: 'memo', count: 6, radius: 90 }, shout: '이것도 좀 같이 봐줘요~' },
      { kind: 'aimed', cooldown: 5, params: { bullets: 3, spreadDeg: 30, speed: 170, damage: 8 }, shout: '보고서 양식이 이게 아니잖아요' },
      { kind: 'charge', cooldown: 9, hpBelow: 0.7, params: { speed: 280, dur: 0.8, windup: 0.8 }, shout: '잠깐 나 좀 봐요!' },
      { kind: 'wall', cooldown: 18, hpBelow: 0.5, params: { enemy: 'meeting', count: 14, radius: 175 }, shout: '긴급 회의 소집! 다들 잠깐 모여봐요' },
  ...
  // 최종 보스
  {
    ...
    abilities: [
      ...
      { kind: 'slam', cooldown: 8, params: { radius: 90, damage: 26, windup: 0.9 }, shout: '이게 최선입니까?' },
      ...
      { kind: 'enrage', cooldown: 1, hpBelow: 0.25, params: { speedMul: 1.35, cdMul: 0.6 }, shout: '오늘 회식이다! 아무도 못 가!' },
    ],
...
const CRUNCH: EnemyDef[] = [
  ...
  // 엘리트
  {
    ...
    elite: true,
    behavior: 'spawner', params: { spawnCd: 3.5, spawnId: 'bug', spawnCount: 4 },
    abilities: [
      { kind: 'ring', cooldown: 6, params: { bullets: 10, speed: 140, damage: 8 }, shout: '502 Bad Gateway' },
    ],
    intro: '서버 다운! 버그를 끝없이 뿜어냅니다. 재부팅(물리)하면 상자!',
  },
  // 중간 보스
  {
    id: 'pm', name: 'PM', sprite: '🙄', tint: '#6b46c1',
    hp: 2400, speed: 52, damage: 22, radius: 40, xp: 70, knockbackResist: 1,
    boss: true, body: { suit: '#6b46c1', tie: '#f59e0b' },
    behavior: 'chase',
    abilities: [
      { kind: 'summon', cooldown: 7.5, params: { enemy: 'bug', count: 8, radius: 90 }, shout: '이거 간단한 거죠? 금방 되죠?' },
  ...
  // 최종 보스
  {
    ...
    hp: 48000, speed: 56, damage: 27, radius: 52, xp: 170, knockbackResist: 1,
    ...
    abilities: [
      ...
      { kind: 'slam', cooldown: 9, hpBelow: 0.7, params: { radius: 100, damage: 30, windup: 0.8 }, shout: '이게 보고서야?!' },
      { kind: 'wall', cooldown: 16, hpBelow: 0.5, params: { enemy: 'meeting', count: 24, radius: 190 }, shout: '긴급 회의 소집! 새벽 2시 전략 회의!' },
      { kind: 'enrage', cooldown: 1, hpBelow: 0.25, params: { speedMul: 1.4, cdMul: 0.55 }, shout: '오늘 아무도 집에 못 간다!!!' },
    ],
...
const DINNER: EnemyDef[] = [
  ...
  // 엘리트
  {
    ...
    elite: true,
    behavior: 'spawner', params: { spawnCd: 3.2, spawnId: 'mini_meat', spawnCount: 4 },
    abilities: [
      { kind: 'hazard', cooldown: 8, params: { count: 2, radius: 45, dps: 10, dur: 4 }, shout: '치이익~ 불판 좀 갈아주세요!' },
    ],
    intro: '달궈진 불판! 고기가 끝없이 구워집니다. 엎으면 상자!',
  },
  // 중간 보스
  {
    id: 'toast_master', name: '건배사 과장', sprite: '🥳', tint: '#b7791f',
    hp: 2600, speed: 52, damage: 23, radius: 42, xp: 80, knockbackResist: 1,
    boss: true, body: { suit: '#92400e', tie: '#facc15' },
    behavior: 'chase',
    abilities: [
      { kind: 'summon', cooldown: 7.5, params: { enemy: 'soju', count: 8, radius: 90 }, shout: '자~ 다들 잔 채우시고!' },
  ...
  // 최종 보스
  {
    ...
    hp: 52000, speed: 56, damage: 28, radius: 54, xp: 190, knockbackResist: 1,
    ...
    abilities: [
      ...
      { kind: 'wall', cooldown: 15, hpBelow: 0.5, params: { enemy: 'soju', count: 26, radius: 190 }, shout: '전원 원샷! 잔 비우면 보내줄게!' },
      { kind: 'enrage', cooldown: 1, hpBelow: 0.25, params: { speedMul: 1.45, cdMul: 0.55 }, shout: '노래방 가자!!! 3차!!!' },
    ],
...
const HOLIDAY: EnemyDef[] = [
  ...
  },
  {
    ...
    behavior: 'chase',
    intro: '"살쪘네?" 묵직하게 다가옵니다. (명절 음식 탓입니다)',
  },
  {
  ...
  },
  {
    ...
    behavior: 'ranged', params: { keep: 210, shotCd: 2.2, shotSpeed: 180, shotDamage: 8 },
    intro: '채널 쟁탈전 발발! 리모컨이 원거리에서 쏩니다.',
  },
  // 엘리트
  {
    id: 'gift', name: '선물세트', sprite: '🎁', tint: '#c53030',
    hp: 360, speed: 28, damage: 14, radius: 24, xp: 40, knockbackResist: 0.9, armor: 2, shield: 100,
    elite: true,
    behavior: 'spawner', params: { spawnCd: 3, spawnId: 'songpyeon', spawnCount: 5 },
    abilities: [
      { kind: 'aimed', cooldown: 6, params: { bullets: 5, spreadDeg: 50, speed: 170, damage: 9 }, shout: '참치 세트 대방출!' },
    ],
    intro: '명절 선물세트! 포장(보호막)을 뜯으면 송편이 쏟아집니다. 부수면 상자!',
  },
  // 중간 보스
  {
    id: 'uncle', name: '삼촌', sprite: '🧔', tint: '#276749',
    hp: 2800, speed: 54, damage: 24, radius: 42, xp: 90, knockbackResist: 1,
    boss: true, body: { suit: '#166534', tie: '#fde047' },
    behavior: 'chase',
    abilities: [
      { kind: 'summon', cooldown: 8, params: { enemy: 'cousin', count: 5, radius: 100 }, shout: '얘들아~ 저기 형아랑 놀아라!' },
      { kind: 'aimed', cooldown: 5, params: { bullets: 5, spreadDeg: 45, speed: 200, damage: 10 }, shout: '삼촌이 다 너 잘되라고 하는 말이야' },
      { kind: 'slam', cooldown: 8, hpBelow: 0.7, params: { radius: 85, damage: 26, windup: 0.8 }, shout: '허허, 많이 컸네! (등짝 토닥)' },
  ...
  // 최종 보스
  {
    ...
    hp: 56000, speed: 58, damage: 30, radius: 56, xp: 210, knockbackResist: 1,
    boss: true, body: { suit: '#9d174d', tie: '#fbbf24' },
    behavior: 'chase',
    abilities: [
      { kind: 'summon', cooldown: 7.5, params: { enemy: 'songpyeon', count: 12, radius: 110 }, shout: '밥은 먹고 다니니? 더 먹어, 더!' },
      { kind: 'ring', cooldown: 5.5, params: { bullets: 22, speed: 170, damage: 12 }, shout: '너 그러다 큰일 난다~' },
      { kind: 'aimed', cooldown: 6.5, params: { bullets: 7, spreadDeg: 60, speed: 210, damage: 11 }, shout: '옆집 애는 대기업 갔다더라' },
      { kind: 'hazard', cooldown: 11, hpBelow: 0.7, params: { count: 5, radius: 60, dps: 15, dur: 6 }, shout: '기름 튄다! 전 부칠 땐 저리 가 있어!' },
      { kind: 'wall', cooldown: 15, hpBelow: 0.5, params: { enemy: 'nag_marry', count: 26, radius: 190 }, shout: '온 가족 소집! 다 같이 한마디씩!' },
      { kind: 'enrage', cooldown: 1, hpBelow: 0.25, params: { speedMul: 1.5, cdMul: 0.5 }, shout: '이모가 다 너 잘되라고 그러는 거야!!!' },
    ],
