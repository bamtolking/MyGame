...
interface Row { stage: string; tier: string; budget: number; runs: BotResult[] }

const PLAN: { stage: string; budgets: number[] }[] = [
  { stage: 'office', budgets: [0, 1500] },
  { stage: 'crunch', budgets: [1500, 5000] },
  { stage: 'dinner', budgets: [5000, 12000] },
  { stage: 'holiday', budgets: [12000, 25000] },
];
  ...
  it('스테이지·투자 단계별 봇 결과', () => {
    ...
    for (const p of PLAN) {
      for (const b of p.budgets) {
        ...
        const runs = SEEDS.map(seed => runBot({ stage: p.stage, seed, meta: stats, maxSeconds: BALANCE.runSeconds + 40 }));
        rows.push({ stage: p.stage, tier: `₩${b.toLocaleString('en-US')}`, budget: b, runs });
      }
    }
    const lines: string[] = [];
    lines.push('# 밸런스 스윕 결과 (헤드리스 봇)');
    lines.push('');
    lines.push(`스테이지 × 복지 투자액 × 시드 ${SEEDS.length}개. 봇 = 자동 회피 + 무난한 선택(시작 무기 우선, 진화 짝 패시브 선호). \`npm run balance\`로 재생성.`);
    lines.push('');
    lines.push('| 스테이지 | 복지 투자 | 칼퇴율 | 평균 생존 | Lv@12시 | Lv@15시 | Lv@18시 | 평균 처치 | 평균 월급 | 첫 진화(초) | 최대 동시 적 | 사망 원인 |');
    lines.push('|---|---|---|---|---|---|---|---|---|---|---|---|');
    for (const r of rows) {
      ...
      lines.push(`| ${r.stage} | ${r.tier} | ${cl}/${r.runs.length} | ${fmt(avg(r.runs.map(x => Math.min(x.t, BALANCE.runSeconds))))}s | ${lv(12)} | ${lv(15)} | ${lv(18)} | ${fmt(avg(r.runs.map(x => x.kills)))} | ${fmt(avg(r.runs.map(x => x.coins)))} | ${evo.length ? fmt(avg(evo)) : '-'} (${evo.length}/${r.runs.length}) | ${fmt(avg(r.runs.map(x => x.maxEnemies)))} | ${causes || '-'} |`);
    }
    lines.push('');
    lines.push('## 판별 상세');
    for (const r of rows) {
      lines.push(`### ${r.stage} ${r.tier}`);
