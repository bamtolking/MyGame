...
export class Renderer {
  ...
  private drawHazards(s: RunState, bi: BiomeDef, x0: number, x1: number): void {
    ...
    for (const h of s.level.hazards) {
      ...
      if (h.broken && s.hurtT > 0.15 && !h.touched) continue;
      ...
      else drawHang(c, h, col, hc, this.time);
    }
  }
...
function drawSpike(c: CanvasRenderingContext2D, h: Hazard, col: string, hc: boolean): void {
  // drawn 3 px wider/taller than the hitbox on each side → never "hit by nothing"
  const x0 = h.x0 - 4, x1 = h.x1 + 4, top = h.y0 - 5, bot = h.y1; const mid = (x0 + x1) / 2;
  c.fillStyle = 'rgba(0,0,0,0.25)'; c.beginPath(); c.ellipse(mid, bot + 1, (x1 - x0) / 2 + 4, 4, 0, 0, Math.PI * 2); c.fill();
  c.fillStyle = col; c.strokeStyle = hc ? '#fff' : shade(col, -0.45); c.lineWidth = hc ? 3 : 2.5;
  c.beginPath(); c.moveTo(x0, bot); c.lineTo(x0 + (x1 - x0) * 0.22, top + 12); c.lineTo(mid - 4, bot - 8); c.lineTo(mid, top); c.lineTo(mid + 4, bot - 8); c.lineTo(x1 - (x1 - x0) * 0.22, top + 12); c.lineTo(x1, bot); c.closePath(); c.fill(); c.stroke();
  c.fillStyle = 'rgba(255,255,255,0.45)'; c.beginPath(); c.moveTo(mid - 1, top + 6); c.lineTo(mid - 5, bot - 12); c.lineTo(mid - 1, bot - 12); c.closePath(); c.fill();
}
function drawTall(c: CanvasRenderingContext2D, h: Hazard, col: string, hc: boolean, t: number): void {
  const x0 = h.x0 - 4, x1 = h.x1 + 4, top = h.y0 - 4, bot = h.y1; const w = x1 - x0;
  c.fillStyle = 'rgba(0,0,0,0.25)'; c.beginPath(); c.ellipse((x0 + x1) / 2, bot + 1, w / 2 + 6, 4, 0, 0, Math.PI * 2); c.fill();
  c.fillStyle = col; c.strokeStyle = hc ? '#fff' : shade(col, -0.45); c.lineWidth = hc ? 3 : 2.5;
  rr(c, x0, top + 14, w, bot - top - 14, 8); c.fill(); c.stroke();
  // warning stripes
  c.save(); rr(c, x0, top + 14, w, bot - top - 14, 8); c.clip();
  c.fillStyle = 'rgba(255,255,255,0.28)'; for (let y = top; y < bot; y += 26) { c.beginPath(); c.moveTo(x0, y + 14); c.lineTo(x1, y); c.lineTo(x1, y + 10); c.lineTo(x0, y + 24); c.closePath(); c.fill(); }
  c.restore();
  // spiky crown
  c.fillStyle = col; c.beginPath(); c.moveTo(x0 - 2, top + 16);
  for (let i = 0; i <= 4; i++) { c.lineTo(x0 + (w * i) / 4, i % 2 ? top + 14 : top - 2 + Math.sin(t * 3 + i) * 1.5); }
  c.lineTo(x1 + 2, top + 16); c.closePath(); c.fill(); c.stroke();
}
function drawHang(c: CanvasRenderingContext2D, h: Hazard, col: string, hc: boolean, t: number): void {
  const x0 = h.x0, x1 = h.x1, bot = h.y1 + 4; const mid = (x0 + x1) / 2;
  const topY = -400;
  c.strokeStyle = hc ? '#fff' : 'rgba(40,30,50,0.85)'; c.lineWidth = 4; c.beginPath(); c.moveTo(mid, topY); c.lineTo(mid, bot - 36); c.stroke();
  const sway = Math.sin(t * 2 + x0 * 0.01) * 0.04;
  c.save(); c.translate(mid, bot - 36); c.rotate(sway);
  c.fillStyle = col; c.strokeStyle = hc ? '#fff' : shade(col, -0.45); c.lineWidth = hc ? 3 : 2.5;
  rr(c, -(x1 - x0) / 2 - 3, 0, x1 - x0 + 6, 36, 10); c.fill(); c.stroke();
  c.fillStyle = 'rgba(255,255,255,0.3)'; rr(c, -(x1 - x0) / 2 + 3, 5, 8, 22, 4); c.fill();
  // teeth on the bottom edge → reads as "don't touch"
  c.fillStyle = hc ? '#fff' : shade(col, -0.3);
  for (let i = 0; i < 4; i++) { const xx = -(x1 - x0) / 2 + 2 + i * ((x1 - x0) / 4); c.beginPath(); c.moveTo(xx, 36); c.lineTo(xx + (x1 - x0) / 8, 44); c.lineTo(xx + (x1 - x0) / 4, 36); c.closePath(); c.fill(); }
  c.restore();
}
