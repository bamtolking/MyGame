// Balance sweep: 3 classes × N seeds of a full play-through (stand-in human follows the story, 6 AI companions).
...
const jobs: [string, number][] = []; for (const cls of ['sword', 'archer', 'shaman']) for (let s = 1; s <= SEEDS; s++) jobs.push([cls, s]);
...
results.sort((a, b) => a.cls.localeCompare(b.cls) || a.seed - b.seed);
...
const cn: Record<string, string> = { sword: '검객', archer: '궁사', shaman: '무녀' };
let md = `# 밸런스 스윕 결과\n\n생성: ${new Date().toISOString().slice(0, 16)} · \`node scripts/balance.ts ${MIN} ${SEEDS}\` · 실행 시간 ${Math.round((Date.now() - t0) / 1000)}초\n\n`;
md += `조건: 오프라인 월드와 동일(AI 동료 6명, 핏빛 달 7분 간격). "사람" 역할은 이야기(퀘스트)를 따라가는 AI가 대신 플레이하며, 장비 장착·합성·강화를 스스로 합니다. **사람 플레이 데이터가 아니므로** 실제 체감과 차이가 있을 수 있습니다.\n\n`;
