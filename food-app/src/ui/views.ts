...
import type { Compound, Effect, Evidence, Food, FoodCategory, Nutrients, Pathway, Step, TagId } from '../data/types';
...
const foodLink = (f: Food) => `#f.${f.id}`;
const compoundLink = (id: CompoundId) => `#c.${id}`;
...
export function evidenceBadge(ev: Evidence): string {
  ...
  return `<a class="ev ev-${ev}" href="#about" title="${esc(EVIDENCE_HELP[ev])}"><span class="dots" aria-hidden="true">${dots}</span>${esc(EVIDENCE_LABEL[ev])}</a>`;
}
...
function askHtml(kind: 'food' | 'compound' | 'ai', id: string, examples: string[]): string {
  return `<section class="ask" id="ask" data-ask-kind="${kind}" data-ask-id="${esc(id)}" hidden>
    <h2>더 물어보기</h2>
    <p class="muted">이 항목에 대해 궁금한 점을 Claude에게 물어볼 수 있어요. 답변마다 근거 수준이 붙습니다.</p>
    <form class="ask-form">
      <label class="sr" for="ask-input">질문</label>
      <input id="ask-input" name="q" type="text" maxlength="300" autocomplete="off" placeholder="${esc(examples[0] ?? '궁금한 점을 적어 주세요')}" />
      <button type="submit" class="btn">묻기</button>
    </form>
    <div class="ask-examples">${examples.map((e) => `<button type="button" class="chip" data-ask-example="${esc(e)}">${esc(e)}</button>`).join('')}</div>
...
function compoundCard(ref: { id: CompoundId; amount?: string; role: string; effect?: Effect }, open: boolean): string {
  ...
  return `<article class="cc fx-edge-${effect}">
    ...
    ${c.pathways.length > 1 ? `<a class="more" href="${compoundLink(c.id)}">다른 경로 ${c.pathways.length - 1}개와 근거 요약 보기 →</a>` : `<a class="more" href="${compoundLink(c.id)}">근거 요약과 참고자료 보기 →</a>`}
  </article>`;
}
...
function sectionNav(items: [string, string][]): string {
  return `<nav class="jump" aria-label="이 페이지 목차">${items.map(([id, label]) => `<a href="#" data-jump="${id}">${esc(label)}</a>`).join('')}</nav>`;
}

type FoodLike = Pick<Food, 'name' | 'en' | 'emoji' | 'category' | 'summary' | 'verdict' | 'basis' | 'serving' | 'nutrients' | 'howToEat' | 'pairings' | 'cautions' | 'myths'>;

function foodBody(f: FoodLike, compoundsHtml: string, grams: number, extraTop = ''): string {
  ...
  return `<article class="page food">
    ...
    ${sectionNav([['sec-nut', '영양성분'], ['sec-cmp', '성분·기전'], ['sec-eat', '먹는 법'], ...(hasCaution ? ([['sec-warn', '주의·오해']] as [string, string][]) : [])])}

    <section id="sec-nut" class="sec">
      <h2>영양성분</h2>
      <div class="nut" data-nut>${nutritionHtml(f, grams)}</div>
    </section>

    <section id="sec-cmp" class="sec">
      <h2>핵심 성분과 몸속 작용</h2>
      <p class="muted">성분마다 몸에 들어와서 일어나는 일을 단계별로 보여줍니다. 성분 이름을 누르면 다른 경로와 근거 요약을 볼 수 있어요.</p>
      <div class="cclist">${compoundsHtml}</div>
    </section>

    <section id="sec-eat" class="sec">
      <h2>먹는 법</h2>
      ${tipsHtml(f.howToEat)}
    ...
    ${hasCaution ? `<section id="sec-warn" class="sec">
      <h2>주의할 점과 흔한 오해</h2>
...
export function foodView(f: Food, grams: number): string {
  const compounds = f.compounds.map((c, i) => compoundCard(c, i === 0)).join('');
  const names = f.compounds.slice(0, 2).map((c) => COMPOUND_BY_ID.get(c.id)?.name ?? COMPOUND_CATALOG[c.id]);
  return `${foodBody(f, compounds, grams)}
    ${askHtml('food', f.id, [`${f.name}은(는) 하루에 얼마나 먹는 게 좋아요?`, `${f.name}의 ${names[0] ?? '성분'}은 조리하면 어떻게 변해요?`, `${f.name}을(를) 피해야 하는 사람은?`])}
    ${sourceNote()}`;
}

export function aiFoodView(f: AiFood, grams: number): string {
  ...
  return `${foodBody(f, compounds, grams, banner)}
    ${askHtml('ai', f.key, [`${f.name}은(는) 하루에 얼마나 먹는 게 좋아요?`, `${f.name}을(를) 피해야 하는 사람은?`])}`;
}

export function foodContext(f: FoodLike & { compounds: { id?: CompoundId; name?: string; role: string }[] }): string {
  const lines = [
    `식품: ${f.name} (${f.en}) — ${f.category}`,
    `요약: ${f.summary}`,
    `평가: ${f.verdict.text}`,
    `영양(${f.basis}): ${Object.entries(f.nutrients).map(([k, v]) => `${NUTRIENT_BY_ID[k as NutrientId]?.name ?? k} ${v}${NUTRIENT_BY_ID[k as NutrientId]?.unit ?? ''}`).join(', ')}`,
    '핵심 성분:',
    ...f.compounds.map((c) => `- ${c.id ? COMPOUND_BY_ID.get(c.id)?.name ?? c.id : c.name}: ${c.role}`),
    '먹는 법:',
    ...f.howToEat.map((s) => `- ${s.title}: ${s.body}`),
    ...(f.cautions?.length ? ['주의:', ...f.cautions.map((s) => `- ${s}`)] : []),
  ];
  return lines.join('\n');
}

export function compoundContext(c: Compound): string {
  return [
    `성분: ${c.name} (${c.en}) — ${c.kind}, ${EFFECT_LABEL[c.effect]}`,
    `요약: ${c.summary}`,
    ...c.pathways.map((p) => `경로 "${p.title}" [${EVIDENCE_LABEL[p.evidence]}]: ${p.steps.map((s) => `${s.title}: ${s.body}`).join(' → ')}`),
    `근거 요약: ${c.evidenceNote}`,
    `많은 식품: ${(FOODS_BY_COMPOUND.get(c.id) ?? []).map((x) => x.food.name).join(', ')}`,
  ].join('\n');
}

// ---------- 성분 ----------

export function compoundView(c: Compound): string {
  ...
  return `<article class="page compound">
    <header class="cmp-head">
      ...
      <h1>${esc(c.name)}</h1>
      <div class="row wrap">${effectPill(c.effect)}${c.tags.map((t) => `<a class="chip small" href="#t.${t}">${esc(TAGS[t])}</a>`).join('')}</div>
    </header>
    <p class="lede">${esc(c.summary)}</p>

    <section class="sec">
      <h2>몸속에서 일어나는 일</h2>
      <div class="pathways">${c.pathways.map((p) => pathwayHtml(p, true)).join('')}</div>
    </section>
    ...
    ${foods.length ? `<section class="sec">
      <h2>이 성분이 들어 있는 식품</h2>
      <div class="foodgrid">${foods.map(({ food, amount }) => `<a class="fcard" href="${foodLink(food)}"><span class="fe" aria-hidden="true">${esc(food.emoji)}</span><span><b>${esc(food.name)}</b>${amount ? `<small>${esc(amount)}</small>` : ''}</span></a>`).join('')}</div>
    </section>` : ''}

    ${c.refs?.length ? `<section class="sec refs"><h2>참고자료</h2>${listHtml(c.refs, 'reflist')}</section>` : ''}
  </article>
  ${askHtml('compound', c.id, [`${c.name}은(는) 음식으로 먹을 때와 보충제로 먹을 때 차이가 있나요?`, `${c.name}의 흡수를 높이려면?`])}`;
}
...
function compoundRow(c: Compound): string {
  return `<a class="crow" href="${compoundLink(c.id)}"><span class="crow-main"><b>${esc(c.name)}</b><small>${esc(firstSentence(c.summary))}</small></span><span class="crow-side">${effectPill(c.effect)}</span></a>`;
}

function foodChip(f: Food): string {
  return `<a class="fcard" href="${foodLink(f)}"><span class="fe" aria-hidden="true">${esc(f.emoji)}</span><span><b>${esc(f.name)}</b><small>${esc(VERDICT_LABEL[f.verdict.tone])}</small></span></a>`;
}

export function searchView(q: string, hits: Hit[], aiAvailable: boolean, saved: AiFood | undefined): string {
  ...
  const aiBox = saved
    ? `<a class="ai-cta saved" href="#a.${encodeURIComponent(saved.key)}"><b>저장된 AI 분석: ${esc(saved.name)}</b><span>이전에 분석한 결과를 엽니다 →</span></a>`
    : aiAvailable
  ...
  return `<article class="page">
    ...
    ${tags.length ? `<section class="sec"><h2>작용</h2><div class="row wrap">${tags.map((t) => `<a class="chip" href="#t.${t}">${esc(TAGS[t])}</a>`).join('')}</div></section>` : ''}
...
const FEATURED: { food: string; compound: CompoundId; q: string }[] = [
  { food: 'blueberry', compound: 'anthocyanin', q: '안토시아닌은 활성산소를 정말 없앨까?' },
  { food: 'kidney-bean', compound: 'lectin', q: '렉틴이 자가면역을 일으킨다는 말, 어디까지 사실일까?' },
  { food: 'canola-oil', compound: 'oxidized-lipids', q: '식물성 기름이 몸에서 염증을 만드는 조건' },
  { food: 'spinach', compound: 'oxalate', q: '시금치 영양은 살리고 옥살산은 줄이는 법' },
];
...
export function homeView(recent: Food[], aiSaved: AiFood[]): string {
  const featured = FEATURED.filter((x) => FOOD_BY_ID.has(x.food))
    .map((x) => {
      ...
      return `<a class="feat" href="${foodLink(f)}"><span class="fe" aria-hidden="true">${esc(f.emoji)}</span><span class="feat-q">${esc(x.q)}</span><span class="feat-meta">${esc(f.name)}${c ? ` · ${esc(c.name)}` : ''}</span></a>`;
    })
    .join('');
  ...
  return `<article class="page home">
    ...
    ${recent.length || aiSaved.length ? `<section class="sec"><h2>최근 본 식품</h2><div class="row wrap">${recent.map((f) => `<a class="chip" href="${foodLink(f)}">${esc(f.emoji)} ${esc(f.name)}</a>`).join('')}${aiSaved.map((a) => `<a class="chip ai" href="#a.${encodeURIComponent(a.key)}">${esc(a.emoji)} ${esc(a.name)} <small>AI</small></a>`).join('')}</div></section>` : ''}
    ...
    <section class="sec">
      <h2>작용별로 찾기</h2>
      <div class="row wrap">${tagCounts.map(([t, n]) => `<a class="chip${t === 'inflammation-risk' || t === 'oxidative-risk' || t === 'autoimmune' ? ' risk' : ''}" href="#t.${t}">${esc(TAGS[t])} <small>${n}</small></a>`).join('')}</div>
    </section>

    <section class="sec">
      <h2>식품 ${FOODS.length}가지</h2>
      <div class="cats">${cats
        .map(([cat, fs]) => `<div class="cat"><h3><a href="#k.${CATEGORY_ORDER.indexOf(cat)}">${esc(cat)}</a></h3><p>${fs.map((f) => `<a href="${foodLink(f)}">${esc(f.name)}</a>`).join('<span aria-hidden="true"> · </span>')}</p></div>`)
        .join('')}</div>
    </section>

    <section class="sec">
      <h2>성분 사전 ${COMPOUNDS.length}가지</h2>
      <p class="muted">식품을 거치지 않고 성분부터 찾아볼 수도 있어요. <a href="#index">성분 전체 보기 →</a></p>
    </section>
  </article>`;
}
...
function sourceNote(): string {
  return `<p class="source">영양성분은 공개 식품성분 DB 기준 대략값이며 품종·조리법에 따라 달라집니다. 교육용 정보로, 진단·치료를 대신하지 않습니다. <a href="#about">근거 수준과 출처</a></p>`;
}

export function notFoundView(): string {
  return `<article class="page"><h1>페이지를 찾을 수 없어요</h1><p><a href="#">처음으로</a></p></article>`;
}

/** 답변 텍스트 → HTML. 문단·"• " 목록·[근거 …] 표시를 처리한다. */
export function answerHtml(text: string): string {
  const evMap = Object.fromEntries(Object.entries(EVIDENCE_LABEL).map(([k, v]) => [v, k])) as Record<string, keyof typeof EVIDENCE_LABEL>;
  const inline = (s: string) =>
    esc(s).replace(/\[(근거 강함|근거 중간|근거 제한적|논쟁 중)\]/g, (_, label: string) => evidenceBadge(evMap[label]));
  const blocks = text.trim().split(/\n{2,}/);
  return blocks
    .map((b) => {
      const lines = b.split('\n').filter((l) => l.trim());
      if (lines.length && lines.every((l) => /^\s*[•\-*]\s/.test(l))) return `<ul class="bullets">${lines.map((l) => `<li>${inline(l.replace(/^\s*[•\-*]\s/, ''))}</li>`).join('')}</ul>`;
