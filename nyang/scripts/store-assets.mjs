...
await asset('asset=feature&lang=ko', 'store/feature-1024x500-ko.png', 1024, 500);
await asset('asset=feature&lang=en', 'store/feature-1024x500-en.png', 1024, 500);

// 게임 장면 (360x780 @3x = 1080x2340), 언어별
const SCENES = {
  ko: [
...
for (const lang of ['ko', 'en']) {
  mkdirSync(`store/raw/${lang}`, { recursive: true });
  mkdirSync(`store/screenshots/${lang}`, { recursive: true });
  const ctx = await browser.newContext({ viewport: { width: 360, height: 780 }, deviceScaleFactor: 3, isMobile: true, hasTouch: true, locale: lang === 'ko' ? 'ko-KR' : 'en-US' });
  for (const [scene] of SCENES[lang]) {
  ...
  await ctx.close();
  console.log('scenes', lang);
  // 캡션 붙인 스토어 스크린샷: 구글 플레이 1080x1920, 앱스토어 6.7" 1290x2796
  for (const [w, h, tag] of [[1080, 1920, 'play'], [1290, 2796, 'ios67']]) {
    for (const [i, [scene, cap, sub]] of SCENES[lang].entries()) {
      const src = 'data:image/png;base64,' + readFileSync(`store/raw/${lang}/${scene}.png`).toString('base64');
      const page = await browser.newPage({ viewport: { width: w, height: h }, deviceScaleFactor: 1 });
      ...
      await page.locator('#c').screenshot({ path: `store/screenshots/${lang}/${tag}-${i + 1}-${scene}.jpg`, type: 'jpeg', quality: 90 });
      await page.close();
    }
    console.log('screenshots', lang, tag);
  }
}
await browser.close();
await server.close();
writeFileSync('store/README.md', `# 스토어 이미지\n\n\`npm run store-assets\`로 다시 만든다.\n\n- icon-1024.png: 앱스토어 아이콘 (불투명)\n- icon-512-play.png: 구글 플레이 아이콘\n- feature-1024x500-ko.png / -en.png: 구글 플레이 그래픽 이미지\n- screenshots/{ko,en}/play-*.jpg: 구글 플레이 휴대폰 스크린샷 1080×1920\n- screenshots/{ko,en}/ios67-*.jpg: 앱스토어 6.7인치 스크린샷 1290×2796\n- raw/: 캡션 없는 원본 화면 1080×2340 (저장소에는 올리지 않음)\n`);
console.log('done');
